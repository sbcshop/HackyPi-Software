...
try:
    ...
    keyboard_layout.write("ipconfig > iptext.txt") # file will be created inside current directory on cmd
    keyboard.send(Keycode.ENTER)
    time.sleep(1)
    
    keyboard_layout.write("netstat > netfile.txt") 
    keyboard.send(Keycode.ENTER)
    time.sleep(1)
    
    keyboard_layout.write("dir") #list the files in directory.
    keyboard.send(Keycode.ENTER)
    time.sleep(1)
    
    inner_rectangle()
    print_onTFT("File", 60, 40)
    print_onTFT("Created!!", 40, 80)
