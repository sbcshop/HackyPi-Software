...
# Code tested and works for MAC based PC/Laptop but can be modified for Other OS
import digitalio
import board
import busio
import terminalio
...
try:
    ...
    time.sleep(0.2)
    keyboard_layout.write("sudo shutdown -h now")
    keyboard.send(Keycode.ENTER)
    time.sleep(0.2)
    keyboard_layout.write("Your Password") #Enter your mac password
    keyboard.send(Keycode.ENTER)
    time.sleep(1)  
    
    inner_rectangle()
    FONTSCALE = 2
    print_onTFT("MAC will", 20, 30)
